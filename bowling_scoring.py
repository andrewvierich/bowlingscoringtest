...
def convert_strings(data_dict):
    '''Converts bowling score strings into numbers'''
...
def calc_score_values(data):
    '''Calculates bowling scores for each frame and totals them'''
    ...
    for id, frames in data.items():
        ...
        while frame_index <= len(frames) -1:
            ...
            #on ninth frame just add all the values, no special spare or strike math
            elif (frame_index == 9):
...
def calc_median(list):
    ...
    if list_len %2 == 0:
        ...
        return round((val_1 + val_2) /2, 2)
    else:
        #odd
        return round(list[int(len(list)/2 - 1)],2)
...
def main():
    ...
    int_dict = convert_strings(parsed_data)
    scores = calc_score_values(int_dict) #prints game # and scores here
